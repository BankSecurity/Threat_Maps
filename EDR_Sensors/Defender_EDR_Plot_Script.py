...
def main():
    # --- 1. Download and read the Defender Sensors data ---
    print("Downloading Defender Sensors data...")
    sensors_url = "https://raw.githubusercontent.com/BankSecurity/Threat_Maps/refs/heads/main/EDR_Sensors/Defender_Sensors_Demo.csv"
    ...
    if 'count_' in sensors_df.columns:
        sensors_df = sensors_df.rename(columns={'count_': 'Total'})
    
    # --- 4. Merge the two dataframes on country name ---
    print("\nMerging datasets...")
    ...
    matched_countries = set(merged_df['country'].unique())
    all_sensor_countries = set(sensors_df['country'].unique())
    ...
    output_html_name = f"Defender_EDR_Sensors_Map_{timestamp}.html"
